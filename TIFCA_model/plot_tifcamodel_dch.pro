function plot_tifcamodel_dch, thefcstr_ptr
...
if ~tag_exist( *thefcstr_ptr, 'ID' ) then begin 
    ...
    dchfc_id = (*thefcstr_ptr).ID
    if ~strcmp(dchfc_id, 'tifca_model_dch', 15) then begin
...
for k=1, 4 do begin
  ...
  for i=0, info[2]-1 do begin
      xx= oi[0, i]
      yy= oi[1, i]
      yrr[*, i] = max_min_forplot( [(thefc[xx, yy, *]-sqrt(thevarfc[xx, yy, *]))/binT^(k-1) $
                                 , (thefc[xx, yy, *]+sqrt(thevarfc[xx, yy, *]))/binT^(k-1)])
  endfor
...
for j=1, 4 do begin
  ...
  xx = oi[0, 0]
  yy = oi[1, 0]
  plot, binT, thefc[xx, yy, *]/binT^(j-1), title="  ", psym=symcat(pi[0], color=fsc_color(ci[0])) $
       , /xlog $ 
  ...
  errplot, binT, (thefc[xx, yy, *]-sqrt(thevarfc[xx, yy, *]))/binT^(j-1), (thefc[xx, yy, *]+sqrt(thevarfc[xx, yy, *]))/binT^(j-1) $
       , color=fsc_color("red"), thick=2.0
  for l=1, info[2]-1 do begin
      xx=oi[0,l]
      yy=oi[1,l]
      oplot, binT, thefc[xx, yy, *]/binT^(j-1), psym=symcat(pi[l], color=fsc_color(ci[l])) $
       , thick = 2. 
      errplot, binT, (thefc[xx, yy, *]-sqrt(thevarfc[xx, yy, *]))/binT^(j-1), (thefc[xx, yy, *]+sqrt(thevarfc[xx, yy, *]))/binT^(j-1) $
       , color=fsc_color("red"), thick=2.0       
  endfor     
