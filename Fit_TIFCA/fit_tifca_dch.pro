function fit_tifca_dch, rebinT, fc, varfc, para, fixed=fixed, nspecies=nspecies, order=order
; a fit on TIFCA for the dual channel
...
nnfc = dblarr(ofc+1, ofc+1, nb)
nnvarfc = dblarr(ofc+1, ofc+1, nb) + 1.d
arr_rebinT = dblarr(ofc+1, ofc+1, nb)
...
for k=1, ofc do begin
  ...
  for i=0, info[2]-1 do begin
      xx= oi[0, i]
      yy= oi[1, i]
      arr_rebinT[xx, yy, *] = binT                 ; an array of rebinT for using 'mpfitfun' 
      nnfc[xx, yy, *] = fc[xx, yy, *]          ;the factorial cumulants from the first .
      nnvarfc[xx, yy, *] = varfc[xx, yy, *]    ;the variances of the factorial cumulants
  endfor
endfor

if keyword_set(nspecies) then begin
    if (np-6)/4 ne nspecies then begin
...
fit_parameters=mpfitfun('fit_tifcamodel_dch', arr_rebinT, nnfc, sqrt(nnvarfc), para, PARINFO=pi, BESTNORM=chi, /quiet)                        
chi2 = chi/dof
thefc= tifca_model_dch(binT, fit_parameters)
gf=calgammafactors(fit_parameters[np-5])

result={ID:'fitresult_of_tifca_dch', rebinT:binT, gf:gf, thefc:thefc, $
